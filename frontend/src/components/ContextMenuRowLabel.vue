...
const row = computed(() => {
  return mainStore.selectedPixiRowLabel?.row
})
...
// watch for selectedPixiRowLabel to change
watch(
  () => mainStore.selectedPixiRowLabel,
  (newValue) => {
    console.log('selectedPixiRowLabel changed to:', newValue)
